...
export default function LuckyWheelWithButtonAndPanel(props: WheelProps) {
  ...
  const prizes = prizesName.map((prizeName, index) => {
    return index % 2
      ? { background, fonts: [{ text: prizeName, top: 25, fontColor: 'white', fontSize }] }
      : {
          background: background2,
          imgs: [{ src: 'posipool-logo-metatags.svg', top: 65, width: 45 }],
          fonts: [{ text: prizeName, top: 25, fontColor: prizeName === 'LOSE' ? '#3d0710' : '#0865a1', fontSize }],
        }
  })
  ...
  return (
    <>
      ...
      >
        <GridItem colSpan={{ base: 1, md: 1, lg: 1 }} order={{ base: 3, md: 1, lg: 1 }}>
          <RouletteWinners rouletteWinner={rouletteWinner} contract={drawResult}></RouletteWinners>
        </GridItem>
        <GridItem colSpan={{ base: 1, md: 1, lg: 1 }} order={{ base: 1, md: 1, lg: 1 }}>
          <Div id="rewardId">
            <LogoContainer>
        ...
        <GridItem colSpan={{ base: 1, md: 1, lg: 1 }} order={{ base: 2, md: 1, lg: 1 }}>
          <RouletteInfoCard
            rouletteWinner={rouletteWinner}
            drawResult={drawResult}
            amountOfSpin={amountOfSpin}
